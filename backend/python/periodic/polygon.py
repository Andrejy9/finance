...
from fetchers import polygonIO
...
class PeriodicDataFetcher:
    ...
    def fetch_all_historical_data(self):
        ...
        per ciascun ticker utilizzando fetch_polygon_full_history. Il processo continua
        fino a quando Polygon.io non restituisce un errore (es. per limiti di richiesta).
        """
        ...
        for i, ticker in enumerate(tickers, start=1):
            ...
            while not completed:
                try:
                    completed = polygonIO.fetch_polygon_full_history(ticker)
                    if completed:
